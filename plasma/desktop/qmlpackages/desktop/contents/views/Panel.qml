...
PlasmaCore.FrameSvgItem {
    ...
    function adjustBorders() {
        ...
        containmentParent.anchors.topMargin = Math.min(root.margins.top, Math.max(1, root.height - units.iconSizes.medium));

        containmentParent.anchors.bottomMargin = Math.min(root.margins.bottom, Math.max(1, root.height - units.iconSizes.medium));

        //Base the left/right margins on height as well, to have a good radial simmetry
        containmentParent.anchors.leftMargin = Math.min(root.margins.left, Math.max(1, root.height - units.iconSizes.medium));
